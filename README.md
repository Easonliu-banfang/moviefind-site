...
4. 记下你的 Worker 地址，形如 `https://moviefind-search.<你的子域>.workers.dev`。
   - **本项目已部署的 Worker 地址**：`https://moviefind-search.17721266011.workers.dev`
     （部署在已登录的 Cloudflare 账号下，即你的账号。）
   - **建议**：绑定一个自定义域名（Cloudflare 免费），国内访问更稳（`*.workers.dev` 偶尔需要梯子）。
   - **部署后务必实测**：在浏览器打开
     `https://moviefind-search.17721266011.workers.dev/api/search?q=狂飙&max=8`
     看 `results` 是否非空。若为空，说明这些国内影视站**封锁了 Cloudflare 数据中心 IP**（很常见），
     需参考下方「已知限制·CF 机房可达性」处理。
...
本搜索代理运行在 **Cloudflare 数据中心**，而许多国内免费影视站会**封锁数据中心 IP**（或自家套了 Cloudflare / 风控默认拦机房出口）。典型表现：`/api/search` 返回 `{"ok":true,"results":[]}`（HTTP 正常，但 0 命中）。

- **排查**：浏览器打开 `/api/probe?q=片名`，逐站查看字段 `ok / status / len / kwInHtml / has`，即可知道哪些站从 CF 能搜到、哪些被拦。
- **应对**（按推荐顺序）：
  1. **精简站点**：只保留从 CF 能搜到的站（在 `SITES` 里删掉被拦的）。
  2. **改纯前端跳转导航**：前端直接拼各站搜索 URL，用户点哪个站就去哪个站搜。完全不跨域抓，无封 IP 问题，但无法实现「有片源才显示 / 按延迟排序」。
  3. **给 Worker 配住宅 IP 出口代理**：能解决封 IP，但有成本与合规考量，非默认方案。
